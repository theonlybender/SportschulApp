...
PrintCertificatesPresenter.Display {
	...
	public PrintCertificatesView(LocalizationConstants constants) {
		...
		Label printInfo = new Label("Es werden nur Mitglieder angezeigt, die die Prüfung bestanden haben.");
		Label printInfo2 = new Label("Diese Informationen werden auf die Urkunden gedruckt: ");
	...
	public VerticalPanel buildParticipantsList() {
		...
		Column<EventParticipant, Boolean> printColumn = new Column<EventParticipant, Boolean>(new CheckboxCell(true)) {
		...
		printColumn.setFieldUpdater(new FieldUpdater<EventParticipant, Boolean>() {
	...
	public void setEvent(Event event) {
		this.titleBar.setText("Urkunden für '" + event.getName() + "' (" + event.getDate() + " - Kosten: " + event.getCosts() +")");
		...
		this.examinersTextBox.setText(examiners.substring(0, examiners.length() - 2));
	}
